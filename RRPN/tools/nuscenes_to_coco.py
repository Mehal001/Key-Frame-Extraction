# from RRPN.lib.pynuscenes.nuscenes_dataset import IS_MY_VERSION
import _init_path
import os
import sys
import pickle
...
from pynuscenes.utils.nuscenes_utils import nuscenes_box_to_coco, nuscene_cat_to_coco, nuscene_cat_to_new_coco
...
def main():
    ...
    if IS_MY_VERSION:
        categories = [['child',      'person' ,  1],
                     ...
                     ['truck',       'vehicle',  7],
                     ['trailer',     'vehicle',  8],
                     ['barrier',     'movable_object', 9],
                     ['traffic_cone','movable_object', 10]
        ]
    else:
    ...
    if IS_MY_VERSION_2:
        nusc = NuScenes(version=nusc_version, dataroot=args.nusc_root, verbose=True)
    ...
    for i in trange(num_samples):
        ...
        for i, cam_sample in enumerate(sample['camera']):
            ...
            for ann in annotations:
                if IS_MY_VERSION:
                    coco_cat, coco_cat_id, coco_supercat, coco_class = nuscene_cat_to_new_coco(ann.name)
                else:
